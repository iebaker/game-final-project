...
/**
 * PerlinSampler is a class which can create a dense grid of a certain (but expandable) intiial width and initial
 * height, with a set vector density, and set dimensionality. Information about Perlin Noise used in this implementation
 * taken from <a href='http://webstaff.itn.liu.se/~stegu/TNM022-2005/perlinnoiselinks/perlin-noise-math-faq.html'> The
 * Perlin Noise Math FAQ</a>
 */
public class PerlinSampler {
	...
	/**
	 * Constructor. Creates the perlin sampler with a set density, width, height, and dimension. To understand this: The
	 * parameter for density sets the side length of a single Perlin grid square. The width sets the number of these
	 * squares in the x dimension and the height sets the number of these squares in the y dimension.
	 * 
	 * @param gridwidth
	 *            The width, a floating point value, of each gridsquare
	 * @param initialwidth
	 *            The number of grid squares in the x dimension
	 * @param initialheight
	 *            The number of grid squares in the y dimension
	 * @param dimension
	 *            The dimension of the random vectors chosen.
	 */
	public PerlinSampler(float gridwidth, int initialwidth, int initialheight, int dimension) {
		grid = new FloatList[initialwidth][initialheight]; // Disgusting. In Java you can't make arrays of generic
															// types.
		random = new Random();
	 ...
	 *            The dimension of the random vectors chosen.
	 * @param seed
	 *            A Seed to be used by the random number generator
	 */
	...
	/**
	 * Populates the gradient grid with random vectors. Vectors are represented as lists of length D where D is the
	...
	/**
	 * Returns a floating point value for the noise function at the point (x, y) by sampling the Perlin noise function.
	 * 
	 * @param x
	 *            The x position at which to sample
	 * @param y
	 *            The y position at which to sample
	...
	public float sample(float x, float y) {
		...
		float ul_influence = dotProduct(ul_gradient,
				subVector(vector_of_point, Arrays.asList(new Float[] { x_index * gridwidth, y_index * gridwidth })));
		float ur_influence = dotProduct(
				ur_gradient,
				subVector(vector_of_point,
						Arrays.asList(new Float[] { (x_index + 1) * gridwidth, y_index * gridwidth })));
		float ll_influence = dotProduct(
				ll_gradient,
				subVector(vector_of_point,
						Arrays.asList(new Float[] { x_index * gridwidth, (y_index + 1) * gridwidth })));
		float lr_influence = dotProduct(
				lr_gradient,
				subVector(vector_of_point,
						Arrays.asList(new Float[] { (x_index + 1) * gridwidth, (y_index + 1) * gridwidth })));
	 ...
	 * 
	 * @param minuend
	 *            The vector from which the subtrahend is subtracted
	 * @param subtrahend
	 *            The vector to be subtracted from the minuend
	 */
	...
	/**
	 * Returns the easing function of arg (which is restricted to the domain [0,1]). The easing function gently pushes a
	 * value between 0 and 1 towards either 0 or 1 depending on which one it is closer to.
	 * 
	 * @param arg
	 *            The argument to be eased
