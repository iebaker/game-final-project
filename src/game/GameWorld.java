...
public class GameWorld extends World {
	...
	public static enum GameState {
		LOSE("Game Over", "You lost... Try again"), PLAYING("...",
				"Still in play"), WIN("Nice Work!", "You won");
	...
	/**
	 * Checks collisions by way of double iteration through all elements - does
	 * special things for certain entities
	 */
		...
		if (!paused && !cutsceneActive && player != null) {
			...
			for (Entity e : entityStack) {
				if (!e.equals(player) && e.shape != null) {
					...
					if (castTo == null
							|| (castTo != null && tmp != null && tmp.minus(src)
									.mag() < castTo.minus(src).mag())) {
						affected = e;
						castTo = tmp;
					}
				}
			...
			if (castTo != null) {
				...
				if (affected.isShootable()) {
					...
					if (affected.getShotsNeeded() < 1) {
						removeEntity(affected);
					}
				}
				line = castTo;
			}
	...
	/**
	 * Begins a new game, making a new level, setting a countdown message for
	 * when the game starts, and loading the level from file
	 * 
	 * @param restitution
	...
	public void newGame(int lvl) {
		...
		if (lvl == 1) {
			setMessage("Game starts in 3", 0.5f);
		}
	...
	public void onDraw(Graphics2D g) {
		...
		if (v != null && player != null) {
			v.setOffset(player.shape.getLocation().minus(
					v.screenPtToGameForOffset(v.getDim().sdiv(2))));
		}
	...
	public void onKeyPressed(KeyEvent e) {
		...
		if (player != null && !lose && !win && !cutsceneActive) {
	...
	/**
	 * Responder for mouse release - shoots bullet to mouse location with left
	 * button if in game and shoots grenade if right button or ctrl-left mouse
	 * button
	 * 
	 * @param e
	 */
	...
	/**
	 * Does a lot of stuff every tick - see breakdown comments in code below -
	 * but splits tick into multiple iterations of 0.005 seconds for consistency
	 */
	@Override
	public void onTick(float secs) {

		// Calculates standard tick - how many + leftover time to counter for
		// later
		...
		for (int i = 0; i < steps; i++) {
			...
			if (countdown > 1) {
				setMessage(message.subSequence(0, message.length() - 1)
						.toString() + (int) countdown, countdown - TICK_LENGTH);
			}
	...
	/**
	 * Set the message for the message display with a countdown or just pause
	 * the game
	 * 
	 * @param msg
	 * @param ct
