 ...
 * 
 * You shouldn't implement this interface or try to construct a LevelData. Instead, use {@link CS195NLevelReader} to
 * read a LevelData from a file, then traverse the LevelData you get and convert it to your game's classes.
 * 
...
public interface LevelData {
	...
	public interface ConnectionData {
		...
		/**
		 * The source output is any string which identifies the event that should activate this connection, e.g.
		 * "onPressed".
		 * 
		...
		/**
		 * The target input is any string which identifies what the target should do as a result of this connection
		 * being activated, e.g. "Close".
		 * 
	...
	public interface EntityData {
		...
		/**
		 * The "class" of an entity is most frequently interpreted as the actual name of the Java class the entity
		 * should be, e.g. "Player".
		 * 
	...
	public interface ShapeData {
		...
		/**
		 * CIRCLE: the center of the circle POLY: the center of the AAB of the polygon BOX: the center of the box
		 * 
		 ...
		 *             if this shape is not a CIRCLE
		 */
		float getRadius();
		
		/**
		 * BOX: the vertices of the box in counter-clockwise order (clockwise if screen coordinates not math) POLY: the
		 * vertices of the polygon in counter-clockwise order (clockwise if screen coordinates not math)
		 * 
		 * @return a list of the vertices of the shape in counter-clockwise order
		 * @throws UnsupportedOperationException
		 *             if this shape is not a BOX or POLY
		 */
		List<Vec2f> getVerts();
		
		/**
		 * CIRCLE: the point (center.x - radius, center.y - radius) POLY: the "min" corner of the AAB of the polygon
		...
		/**
		 * CIRCLE: the point (center.x + radius, center.y + radius) POLY: the "max" corner of the AAB of the polygon
		 * BOX: the "max" corner of the box
		 * 
		 * @return the point on the bounding AAB of the shape with the greatest x and y coordinates (usually lower
		 *         right)
		 */
		Vec2f getMax();
		 ...
		 * 
		 * The only supported types are Circle, Poly, and Box. Points can be implemented as Circles with a "point: true"
		 * property.
		 * 
		 * @author zdavis
		 */
