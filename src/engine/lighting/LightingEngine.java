...
/**
 * A class which contains methods to actually perform lighting cone calculations
 * for each LightSource in a world.
 */
public class LightingEngine {

	/**
	 * This function determines if two line segments intersect each other, and
	 * if they do, the value of the variable intersection is set to the
	...
	private static Vec2f intersect(Vec2f A1, Vec2f A2, Vec2f B1, Vec2f B2) {
		...
		if (within(intX, A1.x, A2.x) && within(intX, B1.x, B2.x)
	...
	/**
	 * Determines whether a value is in between two other values, with no
	 * regards to the order the limiting values will be given.
	 * 
	 * @param a
	...
	public Vec2f approxPointConvert(Vec2f original) {
		return new Vec2f((original.x) / 5, (original.y) / 5);
	}
	 ...
	 * @param targetPoint
	 *            A point to aim the ray at (such that targetPoint - sourcePoint
	 *            is the direction vector of this ray)
	...
	/**
	 * Public-facing function which loops through each light source and computes
	 * its light cones using a sweepline algorithm over the points and segments
	 * in the world.
	 * 
	 * @param world
	 *            The LightWorld object over which to perform lighting
	 *            calculations
	 */
	public void run(LightWorld world) {
		...
		for (LightSource light : world.getLightSources()) {
			...
			// sweepline
			List<Vec2fPair> pointPairs = world.getPointsAndPairs(lightLocation,
					points);
	...
	private List<LightCone> sweep(Vec2f lightLocation) {
		...
		while (true) {
			...
			if (point == prevSegment.getEndPoint()) {
				LightCone lc = new LightCone(lightLocation, point,
						prevSegment.getBeginPoint());
				cones.add(lc);
				...
				Vec2f intersection = intersect(closest.getBeginPoint(),
						closest.getEndPoint(), prevSegment.getBeginPoint(),
						prevSegment.getEndPoint());
				LightCone lc;

				if (intersection != null) {
					lc = new LightCone(lightLocation, intersection,
							prevSegment.getBeginPoint());
					closest.resetBeginPoint(intersection);
				} else {
					rcd.removePoint(point);
					lc = new LightCone(lightLocation, rcd.minPoint(),
							prevSegment.getBeginPoint());
				}
	...
	public void test1(LightWorld world) {
		System.out
				.println("[test1] This test will make sure that the world can correctly return points and pairs");
	...
	/**
	 * Println based test of sorting (use if you have a small number of points
	 * and can easily check by hand)
	 */
	public void test2(LightWorld world) {
		System.out
				.println("[test2] This test will make sure that sorting works.");
		List<Vec2f> points = new ArrayList<Vec2f>();
		LightSource source = world.getLightSources().get(0);
		world.getPointsAndPairs(source.getLocation(), points);

		System.out
				.println("[test2] Points and pairs retrieved, sorting points with AngularComparator");
	...
	public void test5(LightWorld world, Graphics2D g) {
		...
		List<Segment> segs = v2f2Segment(pairs,
				new AngularComparator(source.getLocation()));
		...
		for (Segment s : segs) {
			...
			g.drawLine((int) firstPoint.x, (int) firstPoint.y,
					(int) midPoint.x, (int) midPoint.y);
			g.setColor(Color.BLUE);
			g.drawLine((int) midPoint.x, (int) midPoint.y, (int) secndPoint.x,
					(int) secndPoint.y);
		}
	}
